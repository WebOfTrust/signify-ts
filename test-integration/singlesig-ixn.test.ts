...
describe('singlesig-ixn', () => {
    ...
    test('ixn1', async () => {
        ...
        const keystate0 = (await client1.keyStates().get(name1_id)).at(0)!;
        expect(keystate0).not.toBeNull();

        // ixn
        const result = await client1.identifiers().interact('name1', {});
        await waitOperation(client1, await result.op());

        // local keystate after ixn
        const keystate1 = (await client1.keyStates().get(name1_id)).at(0)!;
        ...
        const keystate2 = (await client2.keyStates().get(contact1_id)).at(0)!;
